...
_find_locks() = _filter_gitwr(_is_lock)
_find_locks(name) = _filter_gitwr((path) -> _has_lock_name(basename(path), name))

function _clear_locks!(;verb::Bool = false)
    _pull_gitwr(;verb, force = false)
    lcks = _find_locks()
    rm.(lcks; force = true)
    _push_gitwr(;verb, force = true)
end
...
function _locked_sync_gitwr!(f::Function, lkname, owner=rand(UInt); verb::Bool = false)
