devland_dir(sys_root::String) = joinpath(gitworkers_dir(sys_root), "devland")

function _create_client_script(; 
        sys_root, client_root, 
        client_script, server_script, remote_url
    )
    client_tfile = joinpath(@__DIR__, "templates", "client_script_template.jl")
...
function _create_server_script(; 
    ...
    server_tfile = joinpath(@__DIR__, "templates", "server_script_template.jl")
...
function gw_create_devland(; 
    ...
    if isempty(remote_url)
        remote_url, _ = GitLinks.create_local_upstream(devland; 
            verbose, branch_name
        )
    end
    ...
    @info("Setting server")
    server_gw = GitWorker(remote_url, server_root)
    gw_setup(server_gw)

    @info("Setting client")
    client_gw = GitWorker(remote_url, client_root)
    gw_setup(client_gw)
